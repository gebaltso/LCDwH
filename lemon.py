...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 25 12:44:53 2019
...
#def sample_graph(G_linklist,node_number,degree_sequence,starting_node,sample_rate,biased=True):
...
#    print(node_in_new_graph_ori_index)
    node_in_new_graph_ori_index = set(node_in_new_graph_ori_index)
    
    
#    print("The size of sampled graph is", len(node_in_new_graph_ori_index))
...
#    print(map_dict, len(map_dict)) #len map = 1000 otan trexei alliws 999
...
#    s = '827'
#    myFile = 'lemonTEST/datasets/lfr/lfrEdgelistN1000MU0.1*.csv'
...
#    file = "lemonTEST/datasets/lfr/lfrEdgelistN1000MU0.1*.txt"
...
#    test_comm = test_comm - 1
    ...
    new_graph, map_dict, map_dict_reverse, sample_rate, new_graph_size  = sample_graph(graph_linklist,node_number,degree,seedset,1,biased=False)
    
    ######### EDW STAMATAEI (STHN PROHGOUMENH ENTOLH)
...
#   detected_comm = seed_expand_auto(graph_linklist,seedset,min_comm_size,max_comm_size,expand_step,subspace_dim=3,walk_steps=3,biased=False)
...
#            writer.writerow(["Seed node", "Community"])
        
        row = [node1]+[node2]+[wName]+[s]+list(detected_comm)
#        row = [s]+list(detected_comm)
        
        writer.writerow(row)
        
        
     # write out the time needed for algorithm to execute   
