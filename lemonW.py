...
def read_edgelist(filename,delimiter=None,nodetype=int):
    ...
    edgeset = set() #edgeset={(46,79),(211,840), ...}
    nodeset = set() #nodeset={0,1,2, ...}
    ...
    with open(filename) as f:
        for line in f.readlines():
            if not line.strip().startswith("#"):
                ...
                ni,nj, w = nodetype(L[0]),nodetype(L[1]),float(L[2]) #w = weight of edge
                ...
                nodeset.add(nj)
                if ni != nj:
                    edgeset.add(swap(ni,nj)) #swap is used to write the edges in descending order. 
...
#    print("graph_linklist = ", node_number)
    
    del nodeset
    
#    print(G.degree(weight='weight')) #keep couples of (node,weighted_degree)
...
#                    print("a= ", a)
    ...
    graph_dict = SortedDict(graph_dict) #len graph_dict = 1000


    degree = [] #len degree = 1000
    for node in range(node_number): # node_number = 1000
    ...
    for node in range(node_number):
        ...
        weighted_degree.append (graph_dict[node])
    
        
    weight_node0 = weighted_degree[0]
    
    weighted_degree.remove(weighted_degree[0])
    
    weighted_degree.append(weight_node0)
...
#    return graph_linklist, node_number, edge_number,degree
    return graph_linklist, node_number, edge_number,weighted_degree
...
#    if len(RW_graph) >= 7000000: htan 7000
    for j in range(30):
...
#    print( "The size of sampled graph is", len(node_in_new_graph))
    ...
    map_dict = {} #for youtube lel map = 7174
    map_dict_reverse = {}
    count = 0
    for i in range(node_number):
        if i in node_in_new_graph_ori_index:
            ...
            count += 1 #for youtube count = 7174


   
    if count == (new_graph_size - 1):
#    if count == 2999:
#    if count == (node_number - 1):
            map_dict[count] = i
            map_dict_reverse[i] = count
...
def seed_expand_auto(G,seedset,min_comm_size,max_comm_size,expand_step=None,subspace_dim=None,walk_steps=None,biased=True):
    ...
    ####G[x]=[0 0 1 0 ...] 1000 items and 1 where ???
...
def lemon(myFile, seedsetFile, file, communityFile, appendFile):
...
#########################################################################################
    ...
    parser.add_option("-f", "--input network file", dest="network_file", default=myFile,
                      help="input file of edge list for clustering [default: example_graphs/amazon/graph]")


    parser.add_option("-g", "--input community ground truth file", dest="groundtruth_community_file", default=communityFile,
                      help="input file of ground truth community membership [default: example_graphs/amazon/community]")

    parser.add_option("--out", "--output file", dest="output_file", default="seperatedExps/datasets/communities/lemonCom.csv",
                      help="output file of detected community [default: output.txt]")

    parser.add_option("--sd", "--input seed set file", dest="seed_set_file", default=seedsetFile,
    ...
    parser.add_option("-C", "--maximal community size", dest="max_comm_size", default=40,
                      help="the maximum size of a single community in the network [default: 400]")

    #default = 1
    parser.add_option("-s", "--sample rate", dest="sample_rate", default=1,
...
#    test_comm = np.array([14833, 42658, 43004, 58660 ,14835, 14836, 14837 ,106584, 115338 ,42659 ,58661 ,106585 ,288614 ,106586 ,14838 ,106587 ,302943 ,14839 ,14840 ,302944 ,115339, 106588 ,106589 ,206424 ,106590 ,42660 ,106591, 42661, 115340, 293641, 106592])
    test_comm = np.array([260,781,656,531,533,288,37,39,807,298,814,174,433,439,824,827,829,706,578,71,711,713,329,327,210,352,485,616,236,371,501,246,638,639])
...
#    print("type of new graph=",type(new_graph), "type of new seedset=",type(new_seedset))
...
#        out.write("# Jaccard Index: "+str(JIndex)+'\n')
    ...
    os.chdir('/Users/georgiabaltsou/Desktop/PhD/Local_exp/seperatedExps/datasets')
        ...
        # write out result
    with open('communities/'+str(appendFile)+'_communities.csv', 'a') as out_file:
              
        writer = csv.writer(out_file, delimiter=';')
        
        if os.stat('communities/'+str(appendFile)+'_communities.csv').st_size == 0:
            writer.writerow(["Algorithm", "Node 1", "Node 2", "Multiplied Weight", "Seed node", "Community"])
        
        
        row = [alg]+[node1]+[node2]+[wName]+[s]+list(detected_comm_ori+1)
#           row = [wName]+[s]+C
        
        writer.writerow(row)
        
    with open('time/time.txt', 'a') as time_file:
        time_file.write('Lemon execution time is:')
