...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 23 14:15:13 2018
...
def findPathsD(G,s,d, excludeSet = None):
    if excludeSet == None:
        excludeSet = set([s])
    else:
        excludeSet.add(s)
...
#gia upologismo tou pinaka geitonwn tou komvou s se distance akrivws d
def find_un_nei_sD(s, d):
    #pinakas adjacent geitonwn tou komvou s
    n_s = []
    N = []      
    for x in G.neighbors(s):   
...
#print("Paths = ", findAllSimplePaths(G, 0, 11))

print("Neighbors = ", findNeighbors(G, 1, 8))

#for e in G.edges():
##    for x in range(0, len(weightTable)):
##        for y in range(0, len(weightTable)):
#            G[e[0]][e[1]] = 3 #weightTable[x][y]
